...
pub const DataSet = struct {
    ...
    // delete rows after X microseconds
    purge_after_us: ?i64 = null,
    ...
    pub fn handle(self: *DataSet, message: Message) !void {
        switch (message) {
            ...
            .purge => self.purge() catch {}, // already logged
        }
    }
    ...
    fn purge(self: *DataSet) !void {
        ...
        var buf: [logdk.MAX_IDENTIFIER_LEN + 1000]u8 = undefined;
        const sql = try std.fmt.bufPrint(&buf, "delete from \"{s}\" where ldk_ts < $1", .{self.name});
        const n = self.conn.exec(sql, .{cutoff}) catch |err| {
            return logdk.dbErr("DataSet.purge", err, &self.conn, self.logger.level(.Error).string("sql", sql).int("cutoff", cutoff));
        };
        if (n > 0) {
            self.logger.level(.Info).ctx("DataSet.purged").int("records", n).log();
        }
    }
...
fn listItemMapDouble(value: Event.Value) ?f64 {
    switch (value) {
        .null => return null,
        .double => |v| return v,
        .tinyint, .smallint, .integer, .bigint, .utinyint, .usmallint, .uinteger => |v| return @floatFromInt(v),
