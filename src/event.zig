...
test "Event: parse simple" {
    ...
    try assertEvent(.{
        .key_1 = Event.Value{ .bool = true },
        .another_key = Event.Value{ .bool = false },
        .key_3 = Event.Value{ .null = {} },
        .key_4 = Event.Value{ .string = "over 9000!!" },
        .a = Event.Value{ .utinyint = 0 },
        .b = Event.Value{ .utinyint = 1 },
        .c = Event.Value{ .ubigint = 6999384751 },
        .d = Event.Value{ .tinyint = -1 },
        .e = Event.Value{ .integer = -867211 }, // ints
        .f1 = Event.Value{ .double = 0.0 },
        .f2 = Event.Value{ .double = -0.0 },
        .f3 = Event.Value{ .double = 99.33929191 },
        .f4 = Event.Value{ .double = -1.49E10 },
    }, event_list.events[0]);
}
...
test "Event: parse list" {
    ...
    defer event_list.deinit();
    try assertEvent(.{
        .a = Event.Value{ .list = .{ .json = "[1, -9000]", .values = @constCast(&[_]Event.Value{ .{ .utinyint = 1 }, .{ .smallint = -9000 } }) } },
        .b = Event.Value{ .list = .{ .json = "[true, 56.78912, null, {\"abc\": \"123\"}]", .values = @constCast(&[_]Event.Value{ .{ .bool = true }, .{ .double = 56.78912 }, .{ .null = {} }, .{ .json = "{\"abc\": \"123\"}" } }) } },
    }, event_list.events[0]);
}
...
test "Event: parse list simple" {
    ...
    defer event_list.deinit();
    try assertEvent(.{ .a = Event.Value{ .list = .{ .json = "[9999, -128]", .values = @constCast(&[_]Event.Value{ .{ .usmallint = 9999 }, .{ .tinyint = -128 } }) } } }, event_list.events[0]);
}
...
test "Event: parse positive integer" {
    ...
    defer event_list.deinit();
    try assertEvent(.{ .pos = Event.Value{ .list = .{ .json = "[0, 1, 255, 256, 65535, 65536, 4294967295, 4294967296, 18446744073709551615]", .values = @constCast(&[_]Event.Value{ .{ .utinyint = 0 }, .{ .utinyint = 1 }, .{ .utinyint = 255 }, .{ .usmallint = 256 }, .{ .usmallint = 65535 }, .{ .uinteger = 65536 }, .{ .uinteger = 4294967295 }, .{ .ubigint = 4294967296 }, .{ .ubigint = 18446744073709551615 } }) } } }, event_list.events[0]);
}
...
test "Event: parse negative integer" {
    ...
    defer event_list.deinit();
    try assertEvent(.{ .neg = Event.Value{ .list = .{ .json = "[-0, -1, -128, -129, -32768 , -32769, -2147483648, -2147483649, -9223372036854775807, -9223372036854775808]", .values = @constCast(&[_]Event.Value{ .{ .double = -0.0 }, .{ .tinyint = -1 }, .{ .tinyint = -128 }, .{ .smallint = -129 }, .{ .smallint = -32768 }, .{ .integer = -32769 }, .{ .integer = -2147483648 }, .{ .bigint = -2147483649 }, .{ .bigint = -9223372036854775807 }, .{ .bigint = -9223372036854775808 } }) } } }, event_list.events[0]);
}
...
test "Event: tryParse list" {
    {
        var l = Event.Value{ .list = .{ .json = "", .values = @constCast(&[_]Event.Value{ .{ .string = "200" }, .{ .bool = true }, .{ .string = "9000!" } }) } };
