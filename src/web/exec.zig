...
test "exec: rows" {
...

    {
        ...
        try tc.web.expectJson(.{
            .cols = &[_][]const u8{ "id", "name" },
            .types = &[_][]const u8{ "integer", "varchar" },
            .rows = &[_][]const std.json.Value{&[_]std.json.Value{ .{ .integer = 1 }, .{ .string = "leto" } }},
        });
    }

    {
        ...
        try tc.web.expectJson(.{
            .cols = &[_][]const u8{ "id", "name" },
            .types = &[_][]const u8{ "integer", "varchar" },
            .rows = &[_][]const std.json.Value{ &[_]std.json.Value{ .{ .integer = 2 }, .{ .string = "ghanima" } }, &[_]std.json.Value{ .{ .integer = 1 }, .{ .string = "leto" } } },
        });
    }
}
...
test "exec: paging" {
...

    {
        ...
        try tc.web.expectJson(.{
            .cols = &[_][]const u8{"id"},
            .types = &[_][]const u8{"integer"},
            .rows = &[_][]const std.json.Value{&[_]std.json.Value{.{ .integer = 1 }}},
            .total = 1,
        });
    }
